...
def replaceAsset(uid, filePath, fileType, filename):
    '''
    Replaces Asset
    sample url: https://api.contentstack.io/v3/assets/{asset_uid}?environment={environment}&relative_urls={boolean_value}
    documentation on endpoint: https://www.contentstack.com/docs/developers/apis/content-management-api/#replace-asset
    '''
    url = '{}v3/assets/{}?relative_urls=false'.format(region, uid)
    with open(filePath, 'rb') as f:
        fileData = f.read()
    files = {"asset[upload]": (filename, fileData, fileType)}
    res = requests.put(url, files=files, headers=managementTokenHeader)
    if res.status_code in (200, 201):
        config.logging.info('Asset Replaced. ({})'.format(filename))
        return res.json()
    return logError('asset', filename, url, res)
